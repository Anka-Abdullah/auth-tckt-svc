...
func (l *Logger) Printf(format string, args ...interface{}) {
	// Parse GORM log format
	message := fmt.Sprintf(format, args...)

	// Filter GORM logs berdasarkan level
	if strings.Contains(message, "[error]") {
		l.Error("GORM Error", nil, "sql", message)
	} else if strings.Contains(message, "[warn]") {
		l.Warn("GORM Warning", "sql", message)
	} else if strings.Contains(message, "SLOW SQL") {
		// Extract duration dari message
		l.Warn("Slow SQL Query", "query", message)
	} else {
		l.Debug("GORM Query", "sql", message)
	}
}
