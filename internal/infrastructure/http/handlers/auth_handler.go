...
type (
	...
	ResetPasswordWithOTPRequest struct {
		Email       string `json:"email" validate:"required,email"`
		OTP         string `json:"otp" validate:"required,len=6"`
		NewPassword string `json:"new_password" validate:"required,min=8"`
	}
...
func (h *AuthHandler) ForgotPassword(c echo.Context) error {
	...
	h.logger.Info("Password reset OTP sent", "email", req.Email)
...
func (h *AuthHandler) ResetPassword(c echo.Context) error {
	...
	// Panggil service untuk reset password dengan OTP
	if err := h.authService.ResetPassword(req.Email, req.NewPassword); err != nil {
		h.logger.Error("Reset password with OTP failed", err, "email", req.Email)
		return utils.HandleError(c, err)
	}
...
func (h *AuthHandler) VerifyOTP(c echo.Context) error {
	...
	if !valid {
		return c.JSON(http.StatusUnauthorized, utils.ErrorResponse("Invalid OTP"))
	}
